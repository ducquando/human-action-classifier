...
# 1) Dataset indexing
# -----------------------
# Action labels for 6-class classification
...
def get_segments_from_txt(txt_path):
    """Parses the 00sequences.txt file to extract frame ranges for action segments."""
    segments_map = {}
    if not os.path.exists(txt_path):
        print(f"Error: {txt_path} file not found.")
        return segments_map

    with open(txt_path, 'r') as f:
        for line in f:
            # Skip empty lines or lines without frame indicators
            if 'frames' not in line or '-' not in line: 
                continue
            
            line = line.strip()
            # Split line to extract the filename (first part)
            parts = re.split(r'\s+', line)
            filename = parts[0]
            
            # Extract all numeric values
            nums = re.findall(r'\d+', line)
            # The first number is usually the person ID; subsequent pairs are frame ranges
            frame_nums = nums[1:] 
            
            valid_ranges = []
            for i in range(0, len(frame_nums), 2):
                if i + 1 < len(frame_nums):
                    valid_ranges.append((int(frame_nums[i]), int(frame_nums[i+1])))
            
            if valid_ranges:
                # Store only 'action' ranges for this specific task
                segments_map[filename] = {
                    'action': valid_ranges
                }
    print(f"Successfully parsed {len(segments_map)} videos from txt.")
    return segments_map

def parse_kth(fn: str):
    """Extracts person_id and class_id from a KTH filename string."""
...
def list_videos(kth_dir: str):
    """Indexes all .avi files in the KTH directory."""
...
def hof_patch(flow_u, flow_v, x, y, patch=16, bins=8):
    """Computes Histogram of Optical Flow (HOF) for a local image patch."""
    h, w = flow_u.shape
    r = patch // 2
...
def hog_patch(img_g, x, y, patch=16, bins=9):
    """Computes Histogram of Oriented Gradients (HOG) for a local image patch."""
    h, w = img_g.shape
    r = patch // 2
...
def extract_descriptors_segmented(video_path, start_f, end_f, 
    ...
    """Extracts features only within the [start_f, end_f] frame segment."""
    cap = cv2.VideoCapture(video_path)
    
    # Move to the start frame (OpenCV uses 0-based indexing)
    ...
    while curr_f < end_f:
        ...
        # Limit descriptor count per segment for memory efficiency
...
def build_codebook(train_desc_list, k=400, seed=0):
    """Trains a k-means clustering model to serve as a visual vocabulary."""
...
def bovw_hist(desc, codebook):
    """Encodes descriptors into a BoVW histogram."""
...
def main(KTH_DIR, txt_path):
    ...
    if len(train_pool) == 0:
       print("Check: Ensure TRAIN_SUBJECTS list matches the person IDs in filenames.")

    # 1) Training Feature Extraction
    print("Extracting training descriptors for 6 actions...")
    train_desc = []
    y_train = []
    ...
    for v_path, pid, y_class in train_pool:
        ...
        # Only extract defined 'action' segments
        for start_f, end_f in segments_map[v_name]['action']:
            d = extract_descriptors_segmented(v_path, start_f, end_f, 
                                             hog_bins=18, hof_bins=16, max_desc=1000)
            if d.size > 0:
                train_desc.append(d)
                y_train.append(y_class)

    # 2) Codebook Generation (Increased k for better representation)
    print(f"Building codebook with {len(train_desc)} segments...")
    codebook = build_codebook(train_desc, k=2000, seed=0)

    # 3) Histogram Encoding (Train)
    ...
    print("Extracting testing descriptors for 6 actions...")
    X_test = []
    y_test = []
    ...
    models = {
        # Baseline: Chi2 kernel approximation + Linear SVM
    ...
    print(f"Train segments count: {len(train_desc)}")
    print(f"Test segments count: {len(X_test)}")

import matplotlib.pyplot as plt

def plot_histograms(video_path):
    """Plots feature histograms for visualization purposes."""
    cap = cv2.VideoCapture(video_path)
    ok, frame1 = cap.read()
    ok, frame2 = cap.read()
    cap.release()
