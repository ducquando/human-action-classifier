...
def _cache_path(cache_dir: str, video_path: str, params: dict) -> str:
    ...
    return str(Path(cache_dir) / f"{base}_{key}.npy")
...
def main(KTH_DIR):
    ...
    y_train = []
    for v,p,y in train:
        d = extract_descriptors(v, cache_dir=r"C:\Users\aabab\kth\cache_hoghof", frame_step=1)
        train_desc.append(d)
        y_train.append(y)
    ...
    # -----------------------
    # 5) Classifier Experiments (RF, MLP, XGB)
    # -----------------------
    # Define models for comparison
    # 모델 정의
    models = {
        # 기존 Baseline: AdditiveChi2Sampler + LinearSVC
    ...
    print(f"Train segments: {len(train_desc)}")
...
def plot_histograms(video_path):
    ...
    hog_h = hog_patch(img2, cx, cy, patch=16, bins=9)
    hof_h = hof_patch(flow[..., 0], flow[..., 1], cx, cy, patch=16, bins=8)
    ...
    ax[0].set_xlabel("Orientation Bin [0 - 180°]")
    ...
    ax[1].set_xlabel("Direction Bin [0 - 360°]")
...
# Example usage:
# plot_histograms(r"C:\Users\aabab\kth\archive\person01_boxing_d1_uncomp.avi")
...
if __name__ == "__main__":
    ...
    sample_video = r"C:\Users\aabab\kth\archive\running\person02_running_d3_uncomp.avi"
